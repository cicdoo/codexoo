...
class AiAssistantToolLog(models.Model):
    _name = "codexoo.tool_log"
    _description = "Codexoo AI Assistant Tool Call Audit Log"
    _order = "create_date desc, id desc"

    session_id = fields.Many2one(
        "codexoo.session", ondelete="set null", index=True)
    ...
    def _record(self, vals):
        ...
        try:
            with odoo.registry(dbname).cursor() as cr:
                ...
                env["codexoo.tool_log"].create(vals)
                cr.commit()
        except Exception:
            _logger.exception("Failed to write codexoo.tool_log")
