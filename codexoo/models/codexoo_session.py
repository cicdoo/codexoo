...
# Built-in web tools (Codex's native web search). Unlike the ORM tools these are
# NOT served by the mcp__odoo__* bridge — they are the CLI's own built-ins,
# disabled by default. They are opt-in per user (granted via
# res.users.codexoo_tool_ids) and deliberately kept OUT of READ_TOOLS so an empty
# selection never auto-grants web access. The runner enables Codex's web search
# (via config) only when granted. (web_fetch has no distinct Codex equivalent;
# Codex's web search covers retrieval, so it is treated as an alias.)
WEB_TOOLS = ["web_fetch", "web_search"]
WEB_TOOL_BUILTINS = {"web_fetch": "web_search", "web_search": "web_search"}
...
# when the codexoo.action_methods config parameter is unset.
DEFAULT_ACTION_METHOD_PATTERNS = ["action_*", "button_*"]

# Unlike Claude Code, the Codex CLI does not let you deny its built-in shell /
# apply_patch tools. Instead the runner constrains Codex with
# `--sandbox read-only --ask-for-approval never` in an isolated scratch cwd, and
# the authoritative boundary stays server-side: every Odoo mutation goes through
# the loopback-only, token-gated tool endpoints and runs under the user's own
# ACLs. So there is no built-in deny list to enumerate here.

# Fallback glob for the codex binary when it is not on PATH and no explicit
# `codexoo.cli_path` is set. Adjust per deployment (npm global, pipx, etc.).
DEFAULT_CLI_GLOB = "/usr/local/bin/codex"

# --- File upload support -------------------------------------------------
# Files the user may attach to a turn. The CLI's native Read tool understands
# all of these (text/code verbatim, images visually, PDFs page-by-page), and it
# is sandboxed to the per-session uploads dir (see runner._write_settings).
UPLOAD_TEXT_EXTS = {
...
# Subdirectory of the scratch dir that holds uploaded files. The Read tool is
# allowed ONLY under this path — never the scratch root, which also holds the
# generated mcp.json (bridge token) and settings.json.
...
class AiAssistantSession(models.Model):
    _name = "codexoo.session"
    _description = "Codexoo AI Assistant Conversation Session"
    ...
    # Independent of the Codex thread id (which Codex only assigns on the first run).
    ...
    # Codex's own conversation/thread id, captured from the `thread.started` event
    # on the first turn and passed to `codex exec resume <thread_id>` thereafter.
    codex_thread_id = fields.Char(
        string="Codex Thread ID", copy=False, index=True,
        help="Thread id Codex assigns on the first turn; used to resume the conversation.",
    )
    ...
    message_ids = fields.One2many(
        "codexoo.message", "session_id", string="Messages")
    ...
    def _get_scratch_dir(self):
        self.ensure_one()
        root = self._config("scratch_root", "/var/lib/odoo/codexoo_scratch")
    ...
    def _effective_tools(self):
        ...
        self.ensure_one()
        return self.user_id.sudo()._codexoo_effective_tools()
    ...
    def _resolve_cli_path(self):
        """Explicit config param, else `codex` on PATH, else newest glob match."""
        explicit = self._config("cli_path")
        if explicit and os.path.exists(explicit):
            return explicit
        which = __import__("shutil").which("codex")
        if which:
            return which
        ...
        if not matches:
            raise UserError(_(
                "Codex CLI not found. Install it on the Odoo server, then either "
                "put it on the service PATH or set the System Parameter "
                "'codexoo.cli_path' to its absolute path.\n\n"
                "Install (any one):\n"
                "  • npm install -g @openai/codex   (Node 18+)\n"
                "  • brew install codex             (macOS)\n"
                "  • download the release binary from "
                "https://github.com/openai/codex/releases and chmod +x it\n\n"
    ...
    def _codex_env(self):
        """Return env vars pointing the Codex CLI at THIS user's private home.

        Each user authenticates with their own ChatGPT subscription (see
        res.users device-auth helpers); Codex stores credentials in
        ``<home_root>/<uid>/.codex/auth.json``. Isolating CODEX_HOME (and HOME)
        per user keeps credentials, config and session state fully separate, and
        lets Codex refresh that user's token in place.
        """
        self.ensure_one()
        user = self.user_id.sudo()
        return {
            "HOME": user._codexoo_home_dir(),
            "CODEX_HOME": user._codexoo_codex_home(create=True),
        }
    ...
    def _verify_bridge_token(self, token):
        ...
        try:
            ...
            return None
        if payload.get("scope") != "codexoo_bridge":
            return None
