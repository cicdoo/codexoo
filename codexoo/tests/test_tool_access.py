...
"""``_codexoo_effective_tools`` is the single source of truth the controller consults
before running any tool. These tests pin the default-deny posture for writes and
the zero-trust override."""
from odoo.tests.common import TransactionCase, tagged

from odoo.addons.codexoo.models.codexoo_session import (
    READ_TOOLS,
    WRITE_TOOLS,
    READONLY_TOOL_SET,
    WEB_TOOLS,
...
class TestToolAccess(TransactionCase):
    ...
    def setUp(self):
        super().setUp()
        self.Tool = self.env["codexoo.tool"]
        self.user = self.env["res.users"].create({
            "name": "Tool Access Tester",
            "login": "codexoo_tool_tester",
        })
    ...
    def _grant(self, *names):
        ...
        self.user.codexoo_tool_ids = [(6, 0, tools.ids)]

    def test_empty_selection_defaults_to_read_tools(self):
        # No explicit grant → exactly the read-only tool set (no writes).
        self.assertEqual(self.user._codexoo_effective_tools(), set(READ_TOOLS))

    def test_write_tool_requires_explicit_grant(self):
        self.assertNotIn("orm_write", self.user._codexoo_effective_tools())
        self._grant("orm_read", "orm_write")
        self.assertIn("orm_write", self.user._codexoo_effective_tools())

    def test_zero_trust_strips_writes_keeps_reads(self):
        self._grant("orm_read", "orm_write", "orm_unlink", "sql_select")
        self.user.codexoo_zero_trust_mode = "on"
        eff = self.user._codexoo_effective_tools()
    ...
    def test_web_tool_grant_survives_zero_trust(self):
        ...
        self.user.codexoo_zero_trust_mode = "on"
        self.assertIn("web_fetch", self.user._codexoo_effective_tools())
    ...
    def test_unknown_tool_names_are_ignored(self):
        ...
        self.assertNotIn("definitely_not_a_tool",
                         self.user._codexoo_effective_tools())

    def test_excluded_models_config(self):
        self.env["ir.config_parameter"].sudo().set_param(
            "codexoo.excluded_models", "res.users, ir.config_parameter")
        excluded = self.env["codexoo.session"]._ai_excluded_models()
        self.assertEqual(excluded, {"res.users", "ir.config_parameter"})

    def test_default_action_method_patterns(self):
        patterns = self.env["codexoo.session"]._ai_action_method_patterns()
