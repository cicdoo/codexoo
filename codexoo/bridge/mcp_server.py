...
Launched by the MCP client (the Codex CLI, via its mcp_servers config) as a child process. It holds
...
def _call_odoo(tool_name, arguments):
    url = "%s/codexoo/tool/%s" % (BASE, tool_name)
    if ODOO_DB:
...
def _handle(msg):
    ...
    if method == "initialize":
        ...
        # Codex) may send a newer version and will DROP this server's tools if
