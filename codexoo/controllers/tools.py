...
class AiAssistantTools(http.Controller):
    ...
    def _guard(self):
        ...
        return request.codexoo_session
    ...
    def _check_tool(self, tool):
        ...
        if tool not in request.codexoo_session._effective_tools():
    ...
    def _check_model(self, model):
        ...
        if model in request.env["codexoo.session"]._ai_excluded_models():
            raise AccessError(
    ...
    def _check_action_method(self, method):
        ...
        patterns = request.env["codexoo.session"]._ai_action_method_patterns()
    ...
    def _audit(self, started, vals):
        ...
        session = getattr(request, "codexoo_session", None)
        if session:
            vals["session_id"] = session.id
        vals["user_id"] = request.env.uid
        request.env["codexoo.tool_log"].sudo()._record(vals)
    ...
    # Tools
    # ------------------------------------------------------------------
    @http.route("/codexoo/tool/model_introspect", type="json",
                auth="codexoo_bridge", methods=["POST"])
    def model_introspect(self, model=None, **kw):
        ...
        def _do():
            ...
            eff = request.codexoo_session._effective_tools()
    ...
    @http.route("/codexoo/tool/orm_search_read", type="json",
                auth="codexoo_bridge", methods=["POST"])
    ...
    @http.route("/codexoo/tool/orm_read", type="json",
                auth="codexoo_bridge", methods=["POST"])
    ...
    @http.route("/codexoo/tool/orm_call", type="json",
                auth="codexoo_bridge", methods=["POST"])
    ...
    @http.route("/codexoo/tool/sql_select", type="json",
                auth="codexoo_bridge", methods=["POST"])
    def sql_select(self, query=None, params=None, max_rows=1000, **kw):
        ...
        def _do():
            ...
            # parameter 'codexoo.sql_statement_timeout_ms' (default 10s).
            try:
                timeout_ms = max(1000, int(request.env["ir.config_parameter"].sudo()
                                           .get_param("codexoo.sql_statement_timeout_ms", 10000)))
    ...
    # ------------------------------------------------------------------
    @http.route("/codexoo/tool/orm_create", type="json",
                auth="codexoo_bridge", methods=["POST"])
    ...
    @http.route("/codexoo/tool/orm_write", type="json",
                auth="codexoo_bridge", methods=["POST"])
    ...
    @http.route("/codexoo/tool/orm_unlink", type="json",
                auth="codexoo_bridge", methods=["POST"])
    ...
    # ------------------------------------------------------------------
    @http.route("/codexoo/tool/orm_action", type="json",
                auth="codexoo_bridge", methods=["POST"])
    ...
    @http.route("/codexoo/tool/run_wizard", type="json",
                auth="codexoo_bridge", methods=["POST"])
    ...
    @http.route("/codexoo/tool/run_server_action", type="json",
                auth="codexoo_bridge", methods=["POST"])
    def run_server_action(self, action=None, ids=None, **kw):
        ...
        def _do():
            ...
            allowed = request.env[
                "codexoo.session"]._ai_allowed_server_action_ids()
